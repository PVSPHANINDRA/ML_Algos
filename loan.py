...
Original file is located at
    https://colab.research.google.com/drive/1vtAezeq4z8gcXcrOYaf3tGQoAk3mefvl
"""
...
ohe = OneHotEncoder()
ohe.fit(df)
temp_df = pd.DataFrame(ohe.fit_transform(df[['Gender', 'Married', 'Education', 'Self_Employed', 'Property_Area', 'Dependents']]).toarray())
encoded_df = df.join(temp_df)
del encoded_df['Gender'], encoded_df['Married'],encoded_df['Education'], encoded_df['Self_Employed'],encoded_df['Property_Area'],encoded_df['Dependents']

#temp_df = pd.DataFrame(ohe.fit_transform(df[['Married']]).toarray())

#encoded_df.join(temp_df)
#del encoded_df['Married']

first_column = encoded_df.pop('Loan_Status')
  
# insert column using insert(position,column_name,first_column) function
encoded_df.insert(0, 'Loan_Status', first_column)

normalized_df=(encoded_df.iloc[:,1:]-encoded_df.iloc[:,1:].min())/(encoded_df.iloc[:,1:].max()-encoded_df.iloc[:,1:].min())
normalized_df.insert(0,'Loan_Status',dataset['Loan_Status'])
normalized_df['Loan_Status'] = dataset['Loan_Status']
...
def bootstrap_data(dataset, n_folds):
  ...
  d1= df[df['Loan_Status']=='N']
  d2= df[df['Loan_Status']=='Y']
  
  n1= d1['Loan_Status'].count()/(d1['Loan_Status'].count()+d2['Loan_Status'].count())
  n2= d2['Loan_Status'].count()/(d1['Loan_Status'].count()+d2['Loan_Status'].count())
  ...
  for i in range(n_folds):
    ...
    for i in range(int(n1*fold_size)):
      temp1 = dataset_copy[dataset_copy['Loan_Status']=='N'].sample()
      fold.append(temp1)
      dataset_copy = dataset_copy.drop(temp1.index)


    for i in range(int(n2*fold_size)):
      temp2 = dataset_copy[dataset_copy['Loan_Status']=='Y'].sample()
      fold.append(temp2)
...
def Forward_propagation(train_instance, Network):
  ...
  #print(input)
  a = np.insert(input, 0, 1).reshape(21,1)
  activation[0] = a
...
def N_Networks(dataset, n_folds, Layers, Neurons, Output):
  ...
  for i in range(n_folds):
    ...
    Accuracies, F1_score = Metrics(predicted_values_final, test['Loan_Status'].to_numpy())
    a.append(Accuracies)
    f.append(F1_score)
    print(a,f)
...
Co = [2.1567218832866361,1.333890774980934,1.3285545857203522,1.3275399141290465,1.3215641543731464,1.3185297682764052,1.1659738072370198,
 1.1616591743054725,
 1.1591885014794057,
 1.141353224587518,
 1.1339645359699195,
...
plt.xlabel('Percentage of Loan Eligibility set')
