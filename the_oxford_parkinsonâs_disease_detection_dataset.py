# -*- coding: utf-8 -*-
"""The Oxford Parkinson’s Disease Detection Dataset.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1QnGRC6iLuVnEaxAkh-gk-8xBf4tjRnEj
"""
...
csv_contents = pd.read_csv('parkinsons.csv', header=None)
dataset = csv_contents.values
class_index = 22
instances = dataset[1:]
num_attr = [i for i in range(0, 22)]
...
rf_execute('Parkinsons', instances, class_index, num_attr, 'ig')
# dt_execute('Parkinsons', instances, class_index, num_attr, 'ig')
...
# instances - 195
#acc, f1_score, reg_costs, err_costs, itr = nn_execute("Parkinsons", instances, class_index, num_attr, hidden_layer,
...
normalized_df=(df.iloc[:, :22]-df.iloc[:, :22].min())/(df.iloc[:, :22].max()-df.iloc[:, :22].min())
normalized_df.insert(0,'Diagnosis',df['Diagnosis'])
normalized_df['Diagnosis'] = df['Diagnosis']
...
def bootstrap_data(dataset, n_folds):
  ...
  d1= df[df['Diagnosis']==0]
  d2= df[df['Diagnosis']==1]
  
  n1= d1['Diagnosis'].count()/(d1['Diagnosis'].count()+d2['Diagnosis'].count())
  n2= d2['Diagnosis'].count()/(d1['Diagnosis'].count()+d2['Diagnosis'].count())
  ...
  for i in range(n_folds):
    ...
    for i in range(int(n1*fold_size)):
      temp1 = dataset_copy[dataset_copy['Diagnosis']==0].sample()
      fold.append(temp1)
      dataset_copy = dataset_copy.drop(temp1.index)


    for i in range(int(n2*fold_size)):
      temp2 = dataset_copy[dataset_copy['Diagnosis']==1].sample()
      fold.append(temp2)
...
def Forward_propagation(train_instance, Network):
  ...
  #print(input)
  a = np.insert(input, 0, 1).reshape(23,1)
  activation[0] = a
...
def N_Networks(dataset, n_folds, Layers, Neurons, Output):
  ...
  for i in range(n_folds):
    ...
    Accuracies, F1_score = Metrics(predicted_values_final, test['Diagnosis'].to_numpy())
    a.append(Accuracies)
    f.append(F1_score)
    #print(a,f)
...
Co = [1.671350123068413, 1.5678856654175395,1.5608284734804267, 1.371024353612544, 1.2365160283662486, 1.1900667266874854, 1.128795355752293,1.094512200991379, 1.0906166775819606,1.06593190530242, 0.9756549841893658,0.9101800706713942,0.7000004842747461]

import matplotlib.pyplot as plt
plt.plot(percentages, Co)
plt.xlabel('Percentage of Parkinsons Disease Detection set')
